...
export function syncHtmlFromContent(content) {
  ...
  // Sync embedded JSON for bulletproof static hosting & offline access
  const initialDataRegex = /<script id="portfolioInitialData" type="application\/json">[\s\S]*?<\/script>/;
  const scriptTag = `<script id="portfolioInitialData" type="application/json">\n${JSON.stringify(content, null, 2)}\n</script>`;
  if (initialDataRegex.test(html)) {
    html = html.replace(initialDataRegex, scriptTag);
  }
